...
var particleCount = 1000,
	...
	linkFade = 100, // link fade-out frames
	linkSpeed = 0.25, // distance a link travels in 1 frame
	glareAngle = 0,
...
Particle.prototype.render = function() {
	var pos = position(this.x, this.y, this.z),
		r = ((this.z * particleSizeMultiplier) + particleSizeBase) * (sizeRatio() / 5000),
		o = this.opacity;
...
function position(x, y, z) {
	return {
		x: (x * canvas.width) + ((((canvas.width / 2) - mouse.x + ((nPos.x - 0.5) * noiseStrength*10)) * z) * motion), //SPEED
		y: (y * canvas.height) + ((((canvas.height / 2) - mouse.y + ((nPos.y - 0.5) * noiseStrength*10)) * z) * motion)
	};
}
